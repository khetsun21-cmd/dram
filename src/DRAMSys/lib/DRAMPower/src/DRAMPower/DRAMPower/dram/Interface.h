...
struct TogglingHandleLastBurst {
    ...
    // Allow initialization with brace-init in implementation
    constexpr TogglingHandleLastBurst(uint64_t length, timestamp_t load, bool was_handled)
        : last_length(length), last_load(load), handled(was_handled) {}
